        ...
        <Card>
            ...
            <div className="mt-4 flex justify-center gap-4 text-muted-foreground">
                {author.social.map(social => (
                    <Link key={social.name} href={social.url} className="hover:text-primary transition-colors">
                        <AnimatedIconWrapper>
                            <span className="material-symbols-outlined !w-5 !h-5">{social.icon}</span>
                        </AnimatedIconWrapper>
                    </Link>
                ))}
            </div>
...
const PopularPostsWidget = async () => {
  ...
  const popularPosts = allPosts.slice(0, 3); // For now, just show the 3 most recent
          ...
          {popularPosts.map((post) => (
            <li key={post.slug} className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">{format(new Date(post.frontmatter.date), 'MMM dd, yyyy')}</p>
              </div>
            </li>
          ))}
        </ul>
